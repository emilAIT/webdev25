        ...
        if username in active_connections:
            ...
            if not active_connections[username]:
                ...
                if username in id_to_username.values():
                    user_id = next(
                        (k for k, v in id_to_username.items() if v == username), None
                    )
                    if user_id:
...
async def generate_chat_token(
    roomId: int, request: Request, db: Session = Depends(get_db)
):
    ...
    is_member = (
        ...
        .filter(
            and_(room_members.c.room_id == roomId, room_members.c.user_id == user.id)
        )
        .first()
        is not None
    )
